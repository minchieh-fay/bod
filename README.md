...
程序只管理 `clientOrderId` 以当前配置币种的基础币种小写名称加 `bod` 开头的订单，例如 `ETHUSDC` 对应 `ethbod1_...`。人类订单不会被撤销或修改。
...
- 启动时优先恢复当前未完成的当前币种程序订单；没有时根据当前交易对净持仓决定方向；空仓时使用 `initial_side`。
...
- `layers` 控制挂单层数，默认 `1`。第 `n` 层的振幅倍数为 `3*n-2`，例如 `1A`、`4A`、`7A`。第 `n` 层数量为基础数量的 `1.5^(n-1)`，例如 `1`、`1.5`、`2.25`。每一层使用包含基础币种和层号的独立订单标识，例如 `ethbod1_...`；单边持仓调整层级时沿用被调整原订单的数量。
